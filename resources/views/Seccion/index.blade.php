            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div class="input-group w-50">
                    <form id="busquedaForm" method="GET" action="{{ route('seccion.index') }}" class="d-flex w-100">
                        ...
                        <input type="text" class="form-control" placeholder="Buscar sección..." name="busquedaSeccion" value="{{ request('busquedaSeccion') }}" id="inputBusqueda" autocomplete="off">
                ...
                @can('create_seccion')
                    <button class="btn btn-primary rounded-pill px-4 d-flex align-items-center ms-3 btn-agregar" data-bs-toggle="modal" data-bs-target="#modalAgregarSeccion" title="Agregar Sección" style="background-color: #134496; font-size: 1.2rem;">
                                        ...
                                        <div class="modal-body px-4 py-4">
                                            <form action="{{ route('seccion.update', $seccion->id) }}" method="POST">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" name="nombre" class="form-control @if(session('modal_editar_id') && session('modal_editar_id') == $seccion->id && $errors->has('nombre')) is-invalid @endif" value="{{ old('nombre', $seccion->nombre) }}" required>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div style="display: none;">
                                                        @foreach($especialidades as $especialidad)
                                                            <input type="checkbox" id="esp-{{ $seccion->id }}-{{ $especialidad->id }}" name="especialidades[]" value="{{ $especialidad->id }}" @if($seccion->especialidades->where('id', $especialidad->id)->where('pivot.condicion', 1)->count() > 0) checked @endif>
                                                        @endforeach
                                                    </div>
